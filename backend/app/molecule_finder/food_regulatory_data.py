...
def lookup_by_cid(cid: int) -> dict | None:
    """Return regulatory entry for a PubChem CID, or None if not found."""
    return REGULATORY.get(cid)


def lookup_by_cas(cas: str) -> dict | None:
    """Return regulatory entry for a CAS number, or None if not found."""
    cid = CAS_TO_CID.get(cas.strip())
    return REGULATORY.get(cid) if cid is not None else None


def lookup_by_name(name: str) -> dict | None:
    """Return regulatory entry by compound name (case-insensitive), or None."""
    cid = NAME_TO_CID.get(name.strip().lower())
    return REGULATORY.get(cid) if cid is not None else None


def get_status_for_compound(cid: int | None, name: str | None) -> dict:
    """Return regulatory status for a compound, trying CID then name.

    Always returns a dict with keys: status, fema, eu_fl, restriction.
    Falls back to 'not_evaluated' if not found in either registry.
    """
    entry = None
    if cid is not None:
        entry = lookup_by_cid(cid)
