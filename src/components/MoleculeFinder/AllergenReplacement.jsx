...
const STEPS = [
  ...
  { n: 3, label: "NSGA-II Optimisation" },
];
...
const AllergenReplacement = () => {
  ...
  const startAnimation = (gens) => {
    ...
    for (let genIdx = 0; genIdx < gens.length; genIdx++) {
      ...
      // prediction: only prev-gen Pareto remains; new dots rise
      phaseTimers.current.push(setTimeout(() => {
        setAnimPhase("prediction");
        setPrevAllDots([]);   // non-Pareto ghosts fade away
      }, offset));
      offset += STEP_DURATION;

      // nsga2: new Pareto colours emerge; prev-gen Pareto ghosts still visible
      phaseTimers.current.push(setTimeout(() => {
        setAnimPhase("nsga2");
      }, offset));
      offset += STEP_DURATION;
                    ...
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {poolMeta.seeds?.map(seed => (
                        <span key={seed} className={`px-1.5 py-0.5 rounded text-[9px] border transition-colors duration-500
                          ${s1 ? "bg-rose-900/20 border-rose-800/30 text-rose-400" : "bg-gray-900 border-gray-800 text-gray-600"}`}>
                          {seed}
                        </span>
                      ))}
                    </div>
