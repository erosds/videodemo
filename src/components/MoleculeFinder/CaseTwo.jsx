...
const CaseTwo = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (meta) setPoolMeta(meta.sweetness ?? null);
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        ...
        setPoolMeta(meta.sweetness ?? null);
      } catch { }
    }, 3000);
  ...
  const handleGeneratePool = async () => {
    await fetch(`${BACKEND}/molecule-finder/candidates/generate/sweetness`, { method: "POST" }).catch(() => { });
    setPoolMeta(prev => ({ ...prev, status: "generating" }));
  };

  const handleDeletePool = async () => {
    await fetch(`${BACKEND}/molecule-finder/candidates/pool/sweetness`, { method: "DELETE" }).catch(() => { });
                ...
                {!poolMeta ? (
                  ...
                  <div className="flex flex-col gap-1.5">
                    <p className="text-[9px] text-gray-600 leading-snug">
                      The pool is assembled from PubChem sweetness-related compounds. Seed molecules and filters are pre-defined based on the optimization target (P(sweet) / MW / logS).
                    </p>
                    <button
                  ...
                  <div className="text-[10px] text-gray-500 flex flex-col gap-0.5">
                    ...
                    <span className="text-[9px] text-gray-600 leading-snug">
                      Selected by ≥{poolMeta.threshold}% structural similarity to {poolMeta.seeds?.join(", ")} — sweet reference compounds used as seeds.
                    </span>
                  </div>
                )}
              </div>
